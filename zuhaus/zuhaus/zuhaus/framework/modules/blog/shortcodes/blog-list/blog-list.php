...
class BlogList implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		vc_map(
			array(
				'name'                      => esc_html__( 'Mikado Blog List', 'zuhaus' ),
				'base'                      => $this->base,
				'icon'                      => 'icon-wpb-blog-list extended-custom-icon',
				'category'                  => esc_html__( 'by MIKADO', 'zuhaus' ),
				'allowed_container_element' => 'vc_row',
				'params'                    => array(
					array(
						'type'        => 'dropdown',
						'param_name'  => 'type',
						'heading'     => esc_html__( 'Type', 'zuhaus' ),
						'value'       => array(
							esc_html__( 'Standard', 'zuhaus' ) => 'standard',
							esc_html__( 'Masonry', 'zuhaus' )  => 'masonry',
							esc_html__( 'Simple', 'zuhaus' )   => 'simple'
						),
						'save_always' => true
					),
					array(
						...
						'heading'    => esc_html__( 'Number of Posts', 'zuhaus' )
					),
					array(
						'type'       => 'dropdown',
						'param_name' => 'number_of_columns',
						'heading'    => esc_html__( 'Number of Columns', 'zuhaus' ),
						'value'      => array(
							esc_html__( 'One', 'zuhaus' )   => '1',
							esc_html__( 'Two', 'zuhaus' )   => '2',
							esc_html__( 'Three', 'zuhaus' ) => '3',
							esc_html__( 'Four', 'zuhaus' )  => '4',
							esc_html__( 'Five', 'zuhaus' )  => '5'
						),
					...
					array(
						...
						'heading'     => esc_html__( 'Space Between Items', 'zuhaus' ),
					...
					array(
						'type'        => 'dropdown',
						'param_name'  => 'orderby',
						'heading'     => esc_html__( 'Order By', 'zuhaus' ),
					...
					array(
						'type'        => 'dropdown',
						'param_name'  => 'order',
						'heading'     => esc_html__( 'Order', 'zuhaus' ),
					...
					array(
						'type'        => 'autocomplete',
						'param_name'  => 'category',
						'heading'     => esc_html__( 'Category', 'zuhaus' ),
						'description' => esc_html__( 'Enter one category slug (leave empty for showing all categories)', 'zuhaus' )
					),
					array(
						'type'       => 'dropdown',
						'param_name' => 'image_size',
						'heading'    => esc_html__( 'Image Size', 'zuhaus' ),
						'value'      => array(
							esc_html__( 'Original', 'zuhaus' )  => 'full',
							esc_html__( 'Square', 'zuhaus' )    => 'zuhaus_mikado_square',
							esc_html__( 'Landscape', 'zuhaus' ) => 'zuhaus_mikado_landscape',
							esc_html__( 'Portrait', 'zuhaus' )  => 'zuhaus_mikado_portrait',
							esc_html__( 'Thumbnail', 'zuhaus' ) => 'thumbnail',
							esc_html__( 'Medium', 'zuhaus' )    => 'medium',
					...
					array(
						...
						'heading'    => esc_html__( 'Title Tag', 'zuhaus' ),
						'value'      => array_flip( zuhaus_mikado_get_title_tag( true, array('p' => 'p') ) ),
						'group'      => esc_html__( 'Post Info', 'zuhaus' )
					),
					array(
						'type'       => 'dropdown',
						'param_name' => 'title_transform',
						'heading'    => esc_html__( 'Title Text Transform', 'zuhaus' ),
						'value'      => array_flip( zuhaus_mikado_get_text_transform_array( true ) ),
						'group'      => esc_html__( 'Post Info', 'zuhaus' )
					),
					array(
						'type'        => 'textfield',
						'param_name'  => 'excerpt_length',
						'heading'     => esc_html__( 'Text Length', 'zuhaus' ),
						'description' => esc_html__( 'Number of characters', 'zuhaus' ),
					...
					array(
						...
						'heading'    => esc_html__( 'Enable Post Info Image', 'zuhaus' ),
					...
					array(
						...
						'heading'    => esc_html__( 'Enable Post Info Section', 'zuhaus' ),
						'value'      => array_flip( zuhaus_mikado_get_yes_no_select_array( false, true ) ),
						'dependency' => Array( 'element' => 'type', 'value'   => array( 'standard', 'boxed', 'masonry' ) ),
						'group'      => esc_html__( 'Post Info', 'zuhaus' )
					),
					array(
						...
						'heading'    => esc_html__( 'Enable Post Info Author', 'zuhaus' ),
						'value'      => array_flip( zuhaus_mikado_get_yes_no_select_array( false, true ) ),
					...
					array(
						...
						'heading'    => esc_html__( 'Enable Post Info Date', 'zuhaus' ),
						'value'      => array_flip( zuhaus_mikado_get_yes_no_select_array( false, true ) ),
						'dependency' => Array( 'element' => 'post_info_section', 'value' => array( 'yes' ) ),
						'group'      => esc_html__( 'Post Info', 'zuhaus' )
					),
					array(
						...
						'heading'    => esc_html__( 'Enable Post Info Category', 'zuhaus' ),
						'value'      => array_flip( zuhaus_mikado_get_yes_no_select_array( false, true ) ),
						'dependency' => Array( 'element' => 'post_info_section', 'value' => array( 'yes' ) ),
						'group'      => esc_html__( 'Post Info', 'zuhaus' )
					),
					array(
						...
						'heading'    => esc_html__( 'Enable Post Info Comments', 'zuhaus' ),
					...
					array(
						...
						'heading'    => esc_html__( 'Enable Post Info Like', 'zuhaus' ),
						'value'      => array_flip( zuhaus_mikado_get_yes_no_select_array( false ) ),
						'dependency' => Array( 'element' => 'post_info_section', 'value' => array( 'yes' ) ),
						'group'      => esc_html__( 'Post Info', 'zuhaus' )
					),
					array(
						...
						'heading'    => esc_html__( 'Enable Post Info Share', 'zuhaus' ),
						'value'      => array_flip( zuhaus_mikado_get_yes_no_select_array( false ) ),
						'dependency' => Array( 'element' => 'post_info_section', 'value' => array( 'yes' ) ),
						'group'      => esc_html__( 'Post Info', 'zuhaus' )
					),
					array(
						...
						'heading'    => esc_html__( 'Pagination Type', 'zuhaus' ),
						'value'      => array(
							esc_html__( 'None', 'zuhaus' )            => 'no-pagination',
							esc_html__( 'Standard', 'zuhaus' )        => 'standard-blog-list',
							esc_html__( 'Load More', 'zuhaus' )       => 'load-more',
							esc_html__( 'Infinite Scroll', 'zuhaus' ) => 'infinite-scroll'
						),
						'group'      => esc_html__( 'Additional Features', 'zuhaus' )
					)
				)
			)
		);
	...
	public function render( $atts, $content = null ) {
		...
		zuhaus_mikado_get_module_template_part( 'shortcodes/blog-list/holder', 'blog', $params['type'], $params );
	...
	public function blogCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['category_title'] ) > 0 ) ? esc_html__( 'Category', 'zuhaus' ) . ': ' . $value['category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $category ) ) {
				...
				if ( ! empty( $category_title ) ) {
					$category_title_display = esc_html__( 'Category', 'zuhaus' ) . ': ' . $category_title;
				}
