...
class ZuhausMikadoLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_zuhaus_mikado_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_zuhaus_mikado_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'zuhaus' );
		
		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'zuhaus' );
		}
...
if ( ! function_exists( 'zuhaus_mikado_create_like' ) ) {
	function zuhaus_mikado_create_like() {
		ZuhausMikadoLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'zuhaus_mikado_create_like' );
}