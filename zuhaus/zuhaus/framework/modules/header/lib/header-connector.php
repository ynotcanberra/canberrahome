...
namespace ZuhausMikado\Modules\Header\Lib;
/**
 * Class ZuhausMikadoHeaderConnector
 *
...
class ZuhausMikadoHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'zuhaus_mikado_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'zuhaus_mikado_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'zuhaus_mikado_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'zuhaus_mikado_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to zuhaus_mikado_title_content_padding filter
	 *
	 * @return int
	 */
