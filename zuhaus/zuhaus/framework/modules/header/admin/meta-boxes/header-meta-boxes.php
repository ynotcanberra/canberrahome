...
if ( ! function_exists( 'zuhaus_mikado_header_types_meta_boxes' ) ) {
	function zuhaus_mikado_header_types_meta_boxes() {
		$header_type_options = apply_filters( 'zuhaus_mikado_header_type_meta_boxes', $header_type_options = array( '' => esc_html__( 'Default', 'zuhaus' ) ) );
		
		return $header_type_options;
	}
}

if ( ! function_exists( 'zuhaus_mikado_get_show_dep_for_header_types_meta_boxes' ) ) {
	function zuhaus_mikado_get_show_dep_for_header_types_meta_boxes() {
		$show_dep_options = apply_filters( 'zuhaus_mikado_header_type_show_meta_boxes', $show_dep_options = array() );
		
		return $show_dep_options;
	}
}

if ( ! function_exists( 'zuhaus_mikado_get_hide_dep_for_header_types_meta_boxes' ) ) {
	function zuhaus_mikado_get_hide_dep_for_header_types_meta_boxes() {
		$hide_dep_options = apply_filters( 'zuhaus_mikado_header_type_hide_meta_boxes', $hide_dep_options = array() );
		
		return $hide_dep_options;
	}
}

if ( ! function_exists( 'zuhaus_mikado_get_hide_dep_for_header_behavior_meta_boxes' ) ) {
	function zuhaus_mikado_get_hide_dep_for_header_behavior_meta_boxes() {
		$hide_dep_options = apply_filters( 'zuhaus_mikado_header_behavior_hide_meta_boxes', $hide_dep_options = array() );
...
if ( ! function_exists( 'zuhaus_mikado_map_header_meta' ) ) {
	function zuhaus_mikado_map_header_meta() {
		$header_type_meta_boxes = zuhaus_mikado_header_types_meta_boxes();
		
		$set_active_global_containers_for_default_value = '#mkdf_menu_area_container';
		
		$header_type_meta_boxes_show_dep = array_merge( array( '' => $set_active_global_containers_for_default_value ), zuhaus_mikado_get_show_dep_for_header_types_meta_boxes() );
		...
		$header_type_meta_boxes_hide_dep     = array_merge( $get_all_containers_except_global_for_default_value, zuhaus_mikado_get_hide_dep_for_header_types_meta_boxes() );
		$header_behavior_meta_boxes_hide_dep = zuhaus_mikado_get_hide_dep_for_header_behavior_meta_boxes();
		
		$header_meta_box = zuhaus_mikado_add_meta_box(
			array(
		...
		zuhaus_mikado_add_meta_box_field(
			array(
				...
				'description'   => esc_html__( 'Select header type layout', 'zuhaus' ),
		...
		//additional area
		do_action( 'zuhaus_mikado_additional_header_area_meta_boxes_map', $header_meta_box );
		
		//top area
		do_action( 'zuhaus_mikado_header_top_area_meta_boxes_map', $header_meta_box );
		
		//menu area
		do_action( 'zuhaus_mikado_header_menu_area_meta_boxes_map', $header_meta_box );
	}
	
	add_action( 'zuhaus_mikado_meta_boxes_map', 'zuhaus_mikado_map_header_meta', 50 );
}