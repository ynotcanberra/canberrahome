...
class ZuhausMikadoIconWidget extends ZuhausMikadoWidget {
	public function __construct() {
		parent::__construct(
			'mkdf_icon_widget',
			esc_html__( 'Mikado Icon Widget', 'zuhaus' ),
			array( 'description' => esc_html__( 'Add icons to widget areas', 'zuhaus' ) )
		);
		...
		$this->params = array_merge(
			...
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'icon_text',
					'title' => esc_html__( 'Icon Text', 'zuhaus' )
				),
				array(
					'type'  => 'textfield',
					'name'  => 'link',
                ...
                array(
                    'type'  => 'textfield',
                    'name'  => 'text_color',
                    'title' => esc_html__( 'Text Color', 'zuhaus' )
                ),
				array(
					'type'    => 'dropdown',
					'name'    => 'target',
					'title'   => esc_html__( 'Target', 'zuhaus' ),
	...
	public function widget( $args, $instance ) {
		...
		$data   = array();
		$data[] = ! empty( $instance['icon_hover_color'] ) ? zuhaus_mikado_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
		$data   = implode( ' ', $data );
		...
		if ( ! empty( $instance['icon_size'] ) ) {
			$holder_styles[] = 'font-size: ' . zuhaus_mikado_filter_px( $instance['icon_size'] ) . 'px';
		}
		...
		if ( ! empty( $instance['icon_text'] ) ) {
			$icon_text_html = '<span class="mkdf-icon-text ' . esc_attr( $icon_text_class ) . '" ' . zuhaus_mikado_get_inline_style( $text_styles ) . '>' . esc_html( $instance['icon_text'] ) . '</span>';
		}
		?>
		
		<a <?php zuhaus_mikado_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo zuhaus_mikado_get_inline_style( $holder_styles ); ?>>
