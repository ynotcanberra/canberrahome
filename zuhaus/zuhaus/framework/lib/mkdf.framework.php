...
class ZuhausMikadoFramework {
	...
	private function __construct() {
		$this->mkdOptions         = ZuhausMikadoOptions::get_instance();
		$this->mkdMetaBoxes       = ZuhausMikadoMetaBoxes::get_instance();
		$this->mkdTaxonomyOptions = ZuhausMikadoTaxonomyOptions::get_instance();
		$this->mkdUserOptions     = ZuhausMikadoUserOptions::get_instance();
	}
	...
	public function setSkin( ZuhausMikadoSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class ZuhausMikadoSkinManager
 *
...
class ZuhausMikadoSkinManager {
	...
	/**
	 * @see ZuhausMikadoSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = 'mikado' ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'zuhaus' );
...
/**
 * Class ZuhausMikadoSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class ZuhausMikadoSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( zuhaus_mikado_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo zuhaus_mikado_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.mkdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'zuhaus' ) ?>';
		</script>
	<?php }
...
class ZuhausMikadoOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $zuhaus_mikado_options ) ) {
			return $zuhaus_mikado_options[ $key ];
...
/*
   Class: ZuhausMikadoAdminPage
   A class that initializes Mikado Admin Page
*/
class ZuhausMikadoAdminPage implements iZuhausMikadoLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new ZuhausMikadoFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iZuhausMikadoRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: ZuhausMikadoMetaBoxes
...
/*
   Class: ZuhausMikadoMetaBox
   A class that initializes Mikado Meta Box
*/
class ZuhausMikadoMetaBox implements iZuhausMikadoLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $zuhaus_mikado_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: ZuhausMikadoTaxonomyOptions
   A class that initializes ZuhausMikado Taxonomy Options
*/
class ZuhausMikadoTaxonomyOptions {
	private static $instance;
...
/*
   Class: ZuhausMikadoTaxonomyOption
   A class that initializes ZuhausMikado Taxonomy Option
*/
class ZuhausMikadoTaxonomyOption implements iZuhausMikadoLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new ZuhausMikadoTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: ZuhausMikadoUserOptions
   A class that initializes ZuhausMikado User Options
*/
class ZuhausMikadoUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: ZuhausMikadoUserOption
   A class that initializes ZuhausMikado User Option
*/
class ZuhausMikadoUserOption implements iZuhausMikadoLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new ZuhausMikadoUserFieldFactory();
		$this->scope   = $scope;
	}
...
if ( ! function_exists( 'zuhaus_mikado_init_framework_variable' ) ) {
	function zuhaus_mikado_init_framework_variable() {
		global $zuhaus_mikado_Framework;
		
		$zuhaus_mikado_Framework = ZuhausMikadoFramework::get_instance();
		$mkdSkinManager                 = new ZuhausMikadoSkinManager();
		$zuhaus_mikado_Framework->setSkin( $mkdSkinManager->getSkin() );
	}
	
	add_action( 'zuhaus_mikado_before_options_map', 'zuhaus_mikado_init_framework_variable' );
}
?>